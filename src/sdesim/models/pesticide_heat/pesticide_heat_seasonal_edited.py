...
def F(t,y):    #Define the vectorised ODE
    ...
    LD50 = 10**(-6)  # Median lethal dose  
    T = 40 # Half-life of pesticide
    # Heatwave on days 130-133, temperature-related death rate is 0 otherwise
    if 0 <= p < 130:
        temp = 20
    if 130 <= p < 134:
        temp = 41
    ...
    if 0 <= p < 92:    
       ...
       # Pesticides are used on days 92 and 176 of each year:
        if 92 <= p < 176: 
            t_1 = p - 92
        if 176 <= p < 184:
            t_1 = p - 176

       # Pesticides are used on days 92, 134 and 176 of each year:
       # t_1 = (p - 92) % 42

        X1 = (2**(- t_1/T))*g*c 
        X2 = (2**(- t_1/T))*m*c
        return np.array([b*(y[3]**2 / (X**2 + y[3]**2))*((y[0]+y[1])**2 / (K(t)**2 + (y[0]+y[1])**2)) - sig1*y[0] + sig2(t)*((y[1])**2 / (y[0]+y[1])) - mu*y[0] - ((X1*y[0])/(X1 + LD50)) - (((temp-20)**12)*(y[0]))/((24**12)+((temp-20)**12)),
                     sig1*y[0] - sig2(t)*((y[1])**2 / (y[0]+y[1])) - phi*y[1] - ((X2*y[1])/(X2 + LD50)) - (((temp-20)**12)*(y[1]))/((24**12)+((temp-20)**12)),
                     d(t)*y[1]*sig3*(y[3]**2 / (l + y[3]**2)) + eps*sig4*(y[3]**2 / (l + y[3]**2))-C*y[2],
                     a(t)*y[2]-d(t)*y[1]*(y[3]**2 / (l + y[3]**2)) - eps*(y[3]**2 / (l + y[3]**2)) - w(t)*y[3]])
    elif 184 <= p < 275:
        b = 500    # Max. eclostion rate
